                                ...
                                <div>
                                    <button type="button" class="btn btn-success add-btn" data-bs-toggle="modal"
                                        id="create-btn" data-bs-target="#showModal"><i
                                            ...
                                            href="{{ route('admin.users.create') }}">Thêm</a></button>
                            ...
                            <table class="table align-middle table-nowrap" id="customerTable">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="sort" data-sort="email">Họ và tên</th>
                                        <th class="sort" data-sort="email">Email</th>
                                        <th class="sort" data-sort="email">Số điện thoại</th>
                                        <th class="sort" data-sort="email">Ảnh đại diện</th>
                                        <th class="sort" data-sort="email">Giới tính</th>
                                        <th class="sort" data-sort="email">Ngày sinh</th>
                                        <th class="sort" data-sort="email">Địa chỉ</th>
                                        <th class="sort" data-sort="email">Chức vụ</th>
                                ...
                                <tbody class="list form-check-all">
                                    @foreach ($users as $item)
                                        <tr>
                                            <td class="text-wrap" style="max-width: 200px;"></td>
                                            <td class="text-wrap" style="max-width: 200px;">{{ $item->id }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">{{ $item->name }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">{{ $item->email }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">{{ $item->tel }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">
                                                <img src="{{ Storage::url($item->avatar) }}" width="80px"
                                                    alt="{{ $item->name }}">
                                            </td>
                                            <td class="text-wrap" style="max-width: 200px;">
                                                {{ $item->gender == 1 ? 'nam' : 'nữ' }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">{{ $item->age }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">{{ $item->address }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">
                                                {{ $item->role_id == 1 ? 'user' : 'admin' }}</td>
                                            <td class="text-wrap" style="max-width: 200px;">{!! $item->status == 1
                                                ? '<span class="bg-success">bình thường</span>'
                                                : '<span class="bg-danger">Bị khóa</span>' !!}</td>
                                            <td class="text-wrap" style="max-width: 200px;">
                                                <a href="{{ route('admin.users.show', $item->id) }}"
                                                    class="btn btn-info">Chi tiết</a>
                                                <br><a href="{{ route('admin.users.edit', $item->id) }}"
                                                    class="btn btn-warning">Sửa</a>
                                                <br><a href="{{ route('admin.users.destroy', $item->id) }}"
